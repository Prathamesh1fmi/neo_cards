...
# PowerShell Scripts
scripts['setup.ps1'] = r'''$ErrorActionPreference = "Stop"
...
scripts['dev.ps1'] = r'''$NODE_DIR = "$PSScriptRoot\tools\node"
...
scripts['build.ps1'] = r'''$NODE_DIR = "$PSScriptRoot\tools\node"
if (Test-Path "$NODE_DIR\node.exe") {
    $env:PATH = "$NODE_DIR;" + $env:PATH
}
npm run tauri build
...
scripts['test.ps1'] = r'''$NODE_DIR = "$PSScriptRoot\tools\node"
if (Test-Path "$NODE_DIR\node.exe") {
    $env:PATH = "$NODE_DIR;" + $env:PATH
}
...
scripts['src-tauri/src/domain/mod.rs'] = r'''pub mod deck;
pub mod note;
pub mod card;
'''
...
scripts['src-tauri/src/domain/note.rs'] = r'''use serde::{Serialize, Deserialize};
...
scripts['src-tauri/src/domain/card.rs'] = r'''use serde::{Serialize, Deserialize};
...
scripts['src-tauri/src/db/mod.rs'] = r'''pub mod connection;
pub mod migrations;
'''
...
scripts['src-tauri/src/infrastructure/deck_repository.rs'] = r'''use rusqlite::{params, Connection};
...
scripts['src-tauri/src/commands/mod.rs'] = r'''pub mod deck_commands;
'''
